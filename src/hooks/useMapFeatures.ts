...
export const useMapFeatures = (
  ...
  const createFeatureStyle = useCallback((
    ...
    const baseColor = isSelected ? "#ff6b35" : isEditing ? "#9333ea" : "#4285f4";
  ...
  const calculateMetadata = useCallback((geometry: Geometry, type: FeatureType) => {
    const metadata: any = {};
  ...
  const createFeatureOverlays = useCallback((
    ...
    const saveOverlay = createOverlay(
      renderToStaticMarkup(<Save className="w-4 h-4 text-white" />),
    ...
    const deleteOverlay = createOverlay(
      renderToStaticMarkup(<X className="w-4 h-4 text-white" />),
    ...
    if (options.showEditControls) {
      editOverlay = createOverlay(
        renderToStaticMarkup(<Edit2 className="w-4 h-4 text-white" />),
    ...
    if (options.showInfoDisplay) {
      const geometry = feature.getGeometry();
      if (geometry) {
        const metadata = calculateMetadata(geometry, type);
        let infoText = "";
        
        if (metadata.area) {
          const area = metadata.area;
          infoText = area >= 1000000
        ...
        if (infoText) {
          ...
          infoElement.className = "px-3 py-1.5 bg-black bg-opacity-75 text-white text-xs rounded-lg shadow-lg border border-gray-600";
  ...
  const updateOverlayPositions = useCallback((featureId: string) => {
  ...
  const selectFeature = useCallback((featureId: string) => {
    ...
    if (selectedFeatureRef.current) {
      const oldFeature = drawnFeaturesRef.current.get(selectedFeatureRef.current);
      if (oldFeature) {
        oldFeature.isSelected = false;
        oldFeature.feature.setStyle(createFeatureStyle(oldFeature.type, false, oldFeature.isEditing));
    ...
    if (newFeature) {
      ...
      newFeature.feature.setStyle(createFeatureStyle(newFeature.type, true, newFeature.isEditing));
  ...
  const deselectAllFeatures = useCallback(() => {
    drawnFeaturesRef.current.forEach((drawnFeature, featureId) => {
      ...
      drawnFeature.feature.setStyle(createFeatureStyle(drawnFeature.type, false, drawnFeature.isEditing));
  ...
  const addFeature = useCallback((
    ...
    const drawnFeature: DrawnFeature = {
      ...
      isSelected: false,
      isEditing: false,
      metadata: geometry ? calculateMetadata(geometry, type) : undefined,
    };
    ...
    return featureId;
  }, [createFeatureOverlays, mapInstance, updateOverlayPositions, calculateMetadata, selectFeature]);
